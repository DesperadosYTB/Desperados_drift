...
local drift_speed_limit = 8500.0
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(1, 83) then -- Change the key here
			driftmode = not driftmode

			if driftmode then
				ShowNotification("~g~Mode Drift Activé by DEV | Desperados~.")
			else
				ShowNotification("~r~Mode Drift Desactivé by DEV | Desperados~.")
			end
		end
